...
# ===========================================================================
#  FUNCTION 5 — detect_railway_communities  (Gap G5 fix)
# ===========================================================================

def detect_railway_communities(algorithm: str = "greedy_modularity") -> dict:
    """
    Detect communities (geographic/operational clusters) in the railway network.

    Reveals which stations naturally cluster together — useful for identifying
    railway zones, regional sub-networks, and regional vulnerability assessment.

    Parameters
    ----------
    algorithm : str
        "greedy_modularity"  — fast, good quality (default)
        "label_propagation"  — very fast, non-deterministic
        "girvan_newman"      — slow but precise (use only on small subgraphs)

    Returns
    -------
    dict:
      num_communities  : int
      communities      : list[dict]  — each with id, stations, size, key_station
      modularity       : float       — quality measure (higher = better separation)
      algorithm        : str
      interpretation   : str
    """
    ...
    if algorithm == "greedy_modularity":
        communities = list(nx_comm.greedy_modularity_communities(G_clean, weight="weight"))
    elif algorithm == "label_propagation":
        communities = list(nx_comm.label_propagation_communities(G_clean))
    elif algorithm == "girvan_newman":
        # Only practical on subgraphs — use top 100 nodes by degree
        top_nodes = sorted(G_clean.degree(), key=lambda x: x[1], reverse=True)[:100]
        sub = G_clean.subgraph([n for n, _ in top_nodes])
        comp = nx_comm.girvan_newman(sub)
    ...
    # Compute modularity
    try:
        modularity = nx_comm.modularity(G_clean, communities)
    except Exception:
        modularity = 0.0

    # Load station importance for key station identification
    try:
        imp_df = pd.read_csv(os.path.join(_PROC, "station_importance.csv"))
        importance_map = dict(zip(imp_df["station_name"], imp_df["betweenness_centrality"]))
    except Exception:
        importance_map = {}
    ...
    for i, comm in enumerate(sorted_comms):
        stations = sorted(list(comm))
        # Key station = highest betweenness in community
        key_station = max(stations, key=lambda s: importance_map.get(s, 0))
        result_communities.append({
            "id":          i + 1,
            "stations":    stations,
            "size":        len(stations),
            "key_station": key_station,
            "centrality":  round(importance_map.get(key_station, 0), 4),
        })

    n = len(sorted_comms)
    interpretation = (
        f"The railway network is structured into {n} operational communities. "
        f"A disruption within any single community is likely to be contained "
        f"locally; cross-community disruptions require more mitigation. "
        f"Modularity score of {modularity:.3f} indicates "
        f"{'strong' if modularity > 0.4 else 'moderate' if modularity > 0.2 else 'weak'} "
